...
describe('ProfileService', () => {
  ...
  describe('getMe', () => {
    ...
    it('should handle error when fetching user profile', async () => {
      const apiClientMock = apiClient as any;
      apiClientMock.get.mockReturnValue(
        throwError(() => new Error('Unauthorized')),
      );

      await expect(lastValueFrom(service.getMe())).rejects.toThrow(
        'Unauthorized',
      );
    });
  });
  ...
  describe('getMyMenu', () => {
    ...
    it('should handle error when fetching menu', async () => {
      ...
      await expect(lastValueFrom(service.getMyMenu())).rejects.toThrow(
        'Forbidden',
      );
    });
  });
